...
_precompiled_apple_resource_bundle = rule(
    ...
    attrs = dicts.add(rule_attrs.common_tool_attrs(), dict(
        infoplists = attr.label_list(
            allow_files = [".plist"],
            default = [
                Label("//Bazel_rules:resource_bundle.plist"),
            ],
        ),
...
def precompiled_apple_resource_bundle_rules_ios(**kwargs):
    _precompiled_apple_resource_bundle(
        environment_plist = "@build_bazel_rules_apple//apple/internal:environment_plist_ios",
        **kwargs
    )
